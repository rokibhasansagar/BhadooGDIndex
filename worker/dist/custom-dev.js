...
<br>
<footer class="text-muted"> <div class="container"> <p class="float-right"> <a href="#">Back to top</a> </p> <p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://t.telegram.ind.in/TheFirstSpeedster">@TheFirstSpeedster</a>, based on Open Source Softwares.</p> <p>&copy; 2020 - <a href="https://hashhackers.com">Hash Hackers</a> and <a href="https://t.telegram.ind.in/BhadooCloud">Bhadoo Cloud Cyber Systems</a></p> </div> </footer>
	`;
  $('body').html(html);
}
...
function nav(path) {
  ...
  <a class="navbar-brand" href="/${cur}:/"><img border="0" alt="Bhadoo Cloud" src="https://cdn.jsdelivr.net/gh/jscdn/svg@1.0.1/bhadoo-cloud-logo.svg" height="30px"></a>
...
function list(path) {
  ...
  <div class="card">
  <h5 class="card-header">Bhadoo Drive: ${document.siteName}</h5>
  ...
  $.post(`/${cur}:id2path`, {id: a_ele.id}, function (data) {
    if (data) {
      ...
      dialog = mdui.dialog({
        ...
        content: `<a class="btn btn-info" href="${href}">Open</a> <a class="btn btn-secondary" href="${href}" target="_blank">Open in New Tab</a> <button class="btn btn-danger" mdui-dialog-cancel>cancel</button><script>dialog.addEventListener('cancel.mdui.dialog', function () {
  console.log('cancel');
});</script>`,
        history: false,
        modal: true,
